					...
					while (true) {
						...
						if (subChoice == 1) {
							...
							String header = "Account Number\tUser Name\tAvail_Balance"
									+ "\tAddress\t\tPhone\t\tCreated_date\t\tLast_Updated_date";
							String detail = login.getAc_number() + "\t\t" + login.getUsername() + "\t\t"
									+ login.getAmount() + "\t\t" + login.getAddress() + "\t\t" + login.getPhone() + "\t"
							...
							if (al.isEmpty()) {
								...
								String header = "SNo\tAC_Number\ttrasaction_date\t\ttype\t\tAmout\t\ttavailable_bal";
								try {
									FileWriter fw = new FileWriter(file);
									fw.write(header);
